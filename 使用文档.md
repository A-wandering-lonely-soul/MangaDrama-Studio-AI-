...
当前版本的后端是 AI Mock 服务，不会请求阿里云接口。
所以现在不需要阿里云 API Key，也可以完整跑通前后端和 AI 流程测试。
...
说明：`dev:core` 与 `dev:server` 现在会在启动前自动清理 3000 端口上的残留进程，减少 `EADDRINUSE` 冲突。
...
说明：该命令已内置端口清理（3000），一般不需要再手动杀进程。
...
请先确认后端已启动（`pnpm dev:server` 或 `pnpm dev:core`），并且 `http://localhost:3000` 未被占用。

3. 后端报错 `EADDRINUSE: address already in use :::3000`

说明 3000 端口已经被其他进程占用，通常是你之前开的 Node 服务还在。
